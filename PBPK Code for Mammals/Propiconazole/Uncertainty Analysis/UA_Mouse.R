...
SA_AUC_mouse          <- SA_AUC[SA_AUC$Species == 'Mouse',]
SA_AUC_mouse  <- SA_AUC_mouse  %>%
...
############################################################################################
#####################  NOW run local sensitivity file until line 280  ######################
############################################################################################
#dev.off()
...
Rblood2plasma_parent.mean          <- 0.579
fub_parent.mean                    <- 0.05 
Emax.mean                          <- 12.7
Vmax_hep_parent.mean               <- 35.22233
Km_hep_parent.mean                 <- 4.4
Vmax_int_parent.mean               <- 2.348156
Km_int_parent.mean                 <- 4.4
ka.mean                            <- 1.16
kdeg.mean                          <- 0.036
...
Km_hep_parent.sd             <- 0.7 * Km_hep_parent.mean 
Vmax_int_parent.sd           <- 0.7 * Vmax_int_parent.mean
Km_int_parent.sd             <- 0.7 * Km_int_parent.mean
ka.sd                        <- 0.5 * ka.mean 
...
idata <- 
  ...
  mutate(
    kdeg = rtruncnorm(
      n = N,
      a = qnorm(0.025, mean = kdeg.mean, sd = kdeg.sd),
      b = qnorm(0.975, mean = kdeg.mean, sd = kdeg.sd),
      mean = kdeg.mean,
      sd = kdeg.sd
...
write.csv(SA_AUC_mouse, "C:/XXX/PPZ/UA/Results/SA_AUC_mouse.csv")
write.csv(stat, "C:/XXX/PPZ/UA/Results/UA_AUC_mouse.csv")
