...
source(paste("C:/XXX/SA/SA_Maternal_8compt_InductionModel_Human.R"))
...
path_sens <- "C:/Users/s1036120/OneDrive - Syngenta/HTTK/Mammals/PPZ/SA/"
...
baseline   <- c(Km_hep_parent     = 4.6,                  
                ...
                Kliver2pu_parent     = 278.7,     
                Klung2pu_parent      = 21.66,   
                Kadipose2pu_parent   = 3961, 
                Kbrain2pu_parent     = 238.9,
                Krest2pu_parent      = 111.9903)
...
dat <- cbind(names,dat)
write.csv(morrisM_i_p,  paste("C:/XXX/PPZ/SA/morris_AUC_blood_parent_human_", oral_mg.kg,"_mg.kg.csv", sep = ''), row.names=FALSE)
...
p  <- ggplot(morrisM_i_p, aes(x = reorder(parameter, -index), y = ((index)) , fill= Time)) +
  ...
  ggtitle(expression(paste('Sensitivity Analysis (Propiconazole) - AUC'[C[blood]], '')))

p

ggsave(paste("Morris_AUCCblood_parent_human_", oral_mg.kg, "_mg.kg.tiff", sep= ''),scale = 1,
       plot = p,
       path = "C:/XXX/PPZ/SA/",
