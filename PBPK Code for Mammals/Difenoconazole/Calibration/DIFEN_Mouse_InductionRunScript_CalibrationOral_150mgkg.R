...
Pred   <- function(pars){
  ...
  Oral_150mg.kg           <- 150                            
  Oral_150mg.kg_input     <- Oral_150mg.kg  * 1000 / MW_parent                                # mg/kg -> umol/kg bw
  Dose_events_15mg.kg_oral <- data.frame(var    = "Agutlumen_parent",                       # Agutlumen_parent; Aven_parent
                                         time   = seq(from = 0, to = ndays * 24, by = 24),
                                         value  = rep(Oral_150mg.kg_input, ndays+1),
                                         method = "add")
  
  out_150mg.kg.oral             <- ode(y = initState, 
                                       times = Times, 
                                       func  = pbpk8cpt, 
  ...
  out_150mg.kg.oral               <- as.data.frame(out_150mg.kg.oral)
  colnames(out_150mg.kg.oral)[1]  <- "Time"
  out_150mg.kg.oral               <- cbind.data.frame (Time             = out_150mg.kg.oral$Time,
                                                      C_blood_parent   = out_150mg.kg.oral$C_blood_parent)    # umol/L
  

  return(list("out_150mg.kg.oral "     = out_150mg.kg.oral))
}
...
MCcost<-function (pars){
  ...
  cost  <- modCost(model=out$out_150mg.kg.oral,   obs= oral_150mg.kg,  weight='std',   x="Time")               # weight std can only be used if there is more than 1 data point
...
system.time(Fit<- modFit(f = MCcost, 
                         ...
                         lower   = c(0.3, 3,  50,  1.5, 6),            # range definement matters
...
'            ka   Km_hep_parent Vmax_hep_parent            EC50            Emax 
      0.6327508       5.8411154      53.0017798       3.1658237      12.2576405'

Sim.fit.out_150mgkg        <- Pred(Fit$par)$out_150mg.kg.oral  
df.Sim.fit.out_150mgkg     <- cbind.data.frame (Time=Sim.fit.out_15mgkg$Time, C_blood_parent   = Sim.fit.out_150mgkg$C_blood_parent)

# plot
ggplot() +
  geom_line (data = df.Sim.fit.out_150mgkg, aes(Time, (C_blood_parent)), col="#00AFBB", lwd=2) + 
  geom_point(data = oral_150mg.kg, aes(Time, C_blood_parent), col = 'red', size=2) + ylab("Conc (umol/L)") +
