...
Pred   <- function(pars){
  ...
  Oral_15mg.kg           <- 15                            
  Oral_15mg.kg_input     <- Oral_15mg.kg  * 1000 / MW_parent                                # mg/kg -> umol/kg bw
  Dose_events_15mg.kg_oral <- data.frame(var    = "Agutlumen_parent",                       # Agutlumen_parent; Aven_parent
                                         time   = seq(from = 0, to = ndays * 24, by = 24),
                                         value  = rep(Oral_15mg.kg_input, ndays+1),
                                         method = "add")
  
  out_15mg.kg.oral             <- ode(y = initState, 
                                       times = Times, 
                                       func  = pbpk8cpt, 
  ...
  out_15mg.kg.oral               <- as.data.frame(out_15mg.kg.oral)
  colnames(out_15mg.kg.oral)[1]  <- "Time"
  out_15mg.kg.oral               <- cbind.data.frame (Time             = out_15mg.kg.oral$Time,
                                                      C_blood_parent   = out_15mg.kg.oral$C_blood_parent)    # umol/L
  

  return(list("out_15mg.kg.oral "     = out_15mg.kg.oral))
}
...
MCcost<-function (pars){
  ...
  cost  <- modCost(model=out$out_15mg.kg.oral,   obs= oral_15mg.kg,  weight='std',   x="Time")               # weight std can only be used if there is more than 1 data point
...
system.time(Fit<- modFit(f = MCcost, 
                         ...
                         lower   = c(0.9, 3,  30,  1.5, 6),            # range definement matters
                         upper   = c(3,   6,  70,    6,  18),  
...
'            ka   Km_hep_parent Vmax_hep_parent            EC50            Emax 
      0.9064289       4.0640128      67.7152792       4.0353718      13.2955883 '

Sim.fit.out_15mgkg        <- Pred(Fit$par)$out_15mg.kg.oral  
df.Sim.fit.out_15mgkg     <- cbind.data.frame (Time=Sim.fit.out_15mgkg$Time, C_blood_parent   = Sim.fit.out_15mgkg$C_blood_parent)

# plot
ggplot() +
  geom_line (data = df.Sim.fit.out_15mgkg, aes(Time, (C_blood_parent)), col="#00AFBB", lwd=2) + 
  geom_point(data = oral_15mg.kg, aes(Time, C_blood_parent), col = 'red', size=2) + ylab("Conc (umol/L)") +
