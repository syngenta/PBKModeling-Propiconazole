...
df  <- as.data.frame(df)
colnames(df)[1] <- "Time"
write.csv(df, paste('C:/XXX/Propiconazole/PBK_Modeling/Reference compounds/Penconazole/Results/Modeling_Pen.oral.', oral_mg.kg, 'mgkg.csv', sep = ''), row.names = FALSE)
...
path     <- "C:/XXX/Propiconazole/PBK_Modeling/Reference compounds/Penconazole/Figures/PEN.xlsx"
...
ggplot() +
  geom_line (data = df, aes(Time, C_plasma_parent,linetype = 'Predicted'), col="#00AFBB", lwd=1) + ylab(expression("Plasma Concentration ("*mu*"mol/L)")) +
...
ggsave(('C:/XXX/Propiconazole/PBK_Modeling/Reference compounds/Penconazole/Results/Rat_50mgkg_oral_0413.tiff'), width = 5, height = 4, dpi = 600, compression = 'lzw')
